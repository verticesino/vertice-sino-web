...
export default function sitemap(): MetadataRoute.Sitemap {
  const homepages: MetadataRoute.Sitemap = ['es','pt','zh'].map(locale => ({ url: `${base}/${locale}`, changeFrequency: 'monthly', priority: locale === 'es' ? 1 : 0.9, alternates: { languages: { es: `${base}/es`, 'pt-BR': `${base}/pt`, 'zh-CN': `${base}/zh`, 'x-default': `${base}/es` } } }));
